...
def like_movie():
    ...
    target_movie = request.form['title_give']
    movie = db.movies.find_one({'title':target_movie})
    ...
    result = db.movies.update_one({'title':target_movie}, {'$set': {'likes': new_likes}})
...
def trash_movie():
    movie_receive = request.form['title_give']
    db.movies.update_one({'title':movie_receive}, {'$set': {'trashed': True}})
...
def restore_movie():
    movie_receive = request.form['title_give']
    db.movies.update_one({'title':movie_receive}, {'$set': {'trashed': False}})
    
    return jsonify({'result': 'success'})
#영구 삭제
@app.route('/api/delete_for_good', methods=['POST'])
def kill_movies():
    # 복구할 영화의 제목을 받아와 db에서 삭제한다.
    receive_movie = request.form['movie_give']
    db.movies.delete_one({'title':receive_movie})
    return jsonify({'result': 'success', 'msg': '영구 삭제되었습니다.'})
